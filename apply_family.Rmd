...
â€? If the result is a list with each element's length equals 1, then it will return a vector.
â€? If the result is a list with vectors of the same length, then it will return a matrix.
â€? Otherwise, it will return a list.
...
**simplify**(cannot be abbreviated): logical/character; a vector, matrix or higher dimensional array. TRUE: returns a vector or matrix, whereas "array": returns an array of â€œrankâ€? (=length(dim(.))) one higher than the result of FUN(X[[i]]).
